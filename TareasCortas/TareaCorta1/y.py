...
# Crear un objeto Event para controlar la grabación
grabar_event = threading.Event()
...
def grabacion(FORMAT, CHANNELS, RATE, CHUNK, audio, archivo):
    ...
    # Guardar datos de audio en un diccionario y ATM
    fix = np.array(audio_data)
    d = fix.tolist()
    fourier_data = fft(d)
    data = {
        "audioData": d,
...
def iniciar():
    ...
    pausado = False
    archivo = "grabacion.wav"  # Cambiamos la extensión a WAV
    grabar_event.clear()  # Reiniciar el evento de grabación
    t1 = threading.Thread(target=grabacion, args=(FORMAT, CHANNELS, RATE, CHUNK, audio, archivo))
    t1.start()
    t2 = threading.Thread(target=cuenta)
    t2.start()

def parar():
    global pausado
    pausado = False
    grabar_event.set()  # Establecer el evento de grabación para detener la grabación
    time.after_cancel(proceso)
    clear_contador()
    bloqueo('normal')
